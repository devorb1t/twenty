---
title: OAuth
description: Authenticate users and access Twenty's API on their behalf using OAuth 2.0.
---

OAuth lets external applications authenticate Twenty users and act on their behalf — without handling passwords. Twenty implements the **OAuth 2.0 Authorization Code** flow with PKCE support, plus **Client Credentials** for server-to-server access.
